...
// Second way
const bigPharmacists02 = pharmacists.map(arrayItems => arrayItems.toUpperCase());
// Second way ends
...
function selectFruits (meyveArray, aradigimOzellik) {
  return meyveArray.filter((arrayElement) => arrayElement.toLowerCase().includes(aradigimOzellik.toLowerCase()));
}
...
const newFullNumber = halfNumbers.reduce( (a,b) => Math.round(a) + Math.round(b) )
...
pharmaImprovementPath().then((içerik) => {
...
function talepEt(meslek) {
  return new Promise((resolve, reject) => {
    ...
    if (meslek === "pharmacist") {
      ...
      reject (`Üzgünüm sayın ${meslek} muhattabım, bir eczacı ile görüşmem var :D`)
    }
  })
}
